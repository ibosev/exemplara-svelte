...
export function createRegion(init: Partial<Omit<Region, 'id'>> = {}): Region {
  return {
    id: createId('region'),
...
export function createPage(init: CreatePageInit = {}): Page {
...
export function createPrintSettings(): PrintSettings {
  return {
    enabled: true,
    header: {
      enabled: true,
      ...
      variants: {
        default:
          '<div class="print-header"><strong>{{document.title}}</strong><span>{{page.label}}</span></div>',
        first: '',
        odd: '',
        even: '',
      },
    ...
    footer: {
      ...
      variants: {
        default:
          '<div class="print-footer"><span>{{date}}</span><span>Page {{page.number}} of {{page.total}}</span></div>',
        first: '',
        odd: '',
        even: '',
      },
...
export function createPrintSection(
  init: Partial<Omit<PrintSection, 'id'>> = {},
): PrintSection {
  return {
    id: createId('section'),
...
export function createPaginationSettings(): PaginationSettings {
...
export function createDocument(init: CreateDocumentInit = {}): ExemplaraDocument {
...
export function clonePageDeep(page: Page): Page {
...
export function cloneNodeDeep(node: ComponentNode): ComponentNode {
  const clone: ComponentNode = JSON.parse(JSON.stringify(node));
  const reassign = (n: ComponentNode): void => {
    n.id = createId('node');

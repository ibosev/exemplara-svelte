...
export function friendlyElementLabel(node: ComponentNode | null | undefined, fallback = 'Block'): string {
  if (!node) return fallback;
  if (node.type === 'html-text' || node.type === 'text') return 'Text';
  if (node.type === 'image') return 'Image';
  if (node.type === 'web-link') return 'Link';
  if (node.type !== 'html-element' && node.type !== 'web-section' && node.type !== 'container') return fallback;
  ...
  if (tag === 'nav' || /\b(nav|menu)\b/.test(className)) return 'Navigation';
  if (tag === 'header' || /\b(header|topbar)\b/.test(className)) return 'Header';
  if (/\bhero\b/.test(className)) return 'Hero';
  if (tag === 'footer' || /\bfooter\b/.test(className)) return 'Footer';
  if (/\bcard\b/.test(className)) return 'Card';
  if (/\b(cta|button|btn)\b/.test(className)) return 'Button';
  if (/\b(gallery|photo|image)\b/.test(className)) return 'Image';
  return TAG_LABELS[tag] ?? fallback;
}

export function tokenStem(name: string): string {
  const normalized = name.toLowerCase();
  return normalized.replace(/^[a-z0-9]+-/, '');
}

export function friendlyTokenLabel(name: string): string {
  const normalized = name.toLowerCase();
  return TOKEN_LABELS[normalized] ?? TOKEN_LABELS[tokenStem(name)] ?? name.replace(/-/g, ' ');
}

export function sortColorTokenEntries(colors: Record<string, string>): Array<[string, string]> {

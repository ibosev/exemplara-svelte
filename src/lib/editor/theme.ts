import type { EditorThemeTokens } from './extension-types.js';
...
} as const satisfies Record<keyof EditorThemeTokens, string>;

/** Render the typed host theme seam as scoped CSS custom properties. */
export function renderEditorThemeStyle(tokens?: EditorThemeTokens): string | undefined {
  if (!tokens) return undefined;
  const declarations: string[] = [];
  for (const key of Object.keys(THEME_PROPERTIES) as (keyof EditorThemeTokens)[]) {
    const value = tokens[key]?.trim();
    if (value) declarations.push(`${THEME_PROPERTIES[key]}: ${value}`);
  }

...
import type { EditorCommandContext } from '../extensions.js';
import {
  IconArrowDown,
  IconArrowUp,
  IconCopy,
...
function targetNode(editor: EditorContext, context: EditorCommandContext) {
  const nodeId = context.nodeId ?? editor.selectedId;
  return nodeId ? findNode(editor.doc, nodeId) : null;
}

function targetNodeId(editor: EditorContext, context: EditorCommandContext): string | null {
  ...
  return {
    ...
    setup(api) {
      ...
      api.addCommand({
        ...
        icon: IconLayers,
        canRun: (editor, context) =>
          !!targetNode(editor, context) && !!editor.extensions.panel('layers'),
        execute: (editor, context) => {
          const nodeId = targetNodeId(editor, context);

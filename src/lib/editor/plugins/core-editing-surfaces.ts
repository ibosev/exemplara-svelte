...
import type { EditorMenuLocation } from '../actions.js';

export function registerCoreEditingMenus(api: EditorExtensionApi): void {
  const menu = (
    id: string,
    commandId: string,
  ...
  ) => api.addMenuItem({ id, commandId, location, group, order, showWhenDisabled });
...
export function registerCoreEditingKeybindings(api: EditorExtensionApi): void {
  const keybinding = (
    id: string,
    commandId: string,
    key: string,
    options: { mod?: boolean; shift?: boolean; alt?: boolean; order?: number } = {},
  ) => api.addKeybinding({ id, commandId, key, ...options });

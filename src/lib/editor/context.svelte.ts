...
import { resolveSectionTemplate, type SectionTemplatePosition } from '../core/print-sections.js';
import {
...
import { createRenderRuntime, type RenderRuntime } from '../renderer/runtime.js';
import {
...
export class EditorContext {
  ...
  leftTab: LeftTab = $state('');
  rightTab: RightTab = $state('');
  /** Compact sidebars retain their vertical icon rail while hiding panel content. */
  leftPanelCompact: boolean = $state(false);
  rightPanelCompact: boolean = $state(false);

  /** Shared Print panel/canvas selection for direct header/footer editing. */
  printPosition: 'header' | 'footer' = $state('header');
  printVariant: PrintTemplateVariant = $state('default');
  printPreviewPage: number = $state(0);
  /** True while the active paper margin iframe is directly contenteditable. */
  printEditing: boolean = $state(false);
  /** Sheets containing an indivisible first block taller than the body frame. */
  blockedFlowPageIds: string[] = $state([]);
  /** Print preview overlay. */
  previewOpen: boolean = $state(false);
  /** Focused data-source and binding workspace opened from the compact toolbar. */
  dataWorkspaceOpen: boolean = $state(false);
  ...
  layerRevealRevision: number = $state(0);
  ...
  dataPreviewEnabled: boolean = $state(true);
  /** Invalidates host theme getters after the host toggle callback runs. */
  hostThemeRevision: number = $state(0);
  /** Complete historical document rendered by the canvas without replacing the editable engine state. */
  historicalPreviewDocument: ExemplaraDocument | null = $state.raw(null);
  /** Durable version identifier associated with the active historical preview. */
  historicalPreviewVersionId: string | null = $state(null);
  /** Bound by Canvas so zoomToFit can measure the viewport. */
  canvasSurface: HTMLElement | null = $state(null);
  hostStatus: EditorStatus | null = $state(null);

  readonly doc: ExemplaraDocument = $derived.by(() => {
    this.engineRevision;
    return this.historicalPreviewDocument ?? this.engine.doc;
  });

  readonly historicalPreviewActive = $derived(this.historicalPreviewDocument !== null);

  readonly canUndo: boolean = $derived.by(() => {
    this.engineRevision;
    return this.engine.canUndo;
  });

  readonly isPrintTabActive = $derived(this.rightTab === 'print');
   ...
   */
  readonly effectivePrintPreviewPage = $derived(
    this.isPrintTabActive
      ? Math.min(this.activePageIndex, Math.max(0, this.doc.pages.length - 1))
      : this.printPreviewPage
  );

  readonly canRedo: boolean = $derived.by(() => {
    this.engineRevision;
    return this.engine.canRedo;
  });

  readonly registryCategories = $derived.by(() => {
  ...
  readonly capabilityDiagnostics: UnsupportedDocumentCapability[] = $derived.by(() => {
  ...
  readonly activePage: Page | null = $derived.by(() => {
    const pages = this.doc.pages;
    return pages[Math.min(this.activePageIndex, pages.length - 1)] ?? null;
  });

  readonly selectedNode: ComponentNode | null = $derived.by(() =>
    this.selectedId ? findNode(this.doc, this.selectedId) : null,
  );

  readonly selectedDefinition: ComponentDefinition | null = $derived.by(() =>
    this.selectedNode ? (this.getDefinition(this.selectedNode.type) ?? null) : null,
  );
      ...
      ? composeEditor({
          ...
          plugins: [
            ...
            ...init.extensions.map((extension, index) =>
              adaptEditorExtension(extension, `legacy.context-extension.${index + 1}`)),
          ],
  ...
  autocompleteSuggestions(trigger: BindingAutocompleteTrigger): EditorAutocompleteSuggestion[] {
    ...
    const formatterRank = (name: string, label: string, description: string): number => {
      if (!query) return 0;
    ...
    const base: EditorAutocompleteSuggestion[] = trigger.kind === 'path'
      ? []
    ...
    for (const suggestion of [...base, ...provided]) unique.set(suggestion.id, suggestion);
  ...
  canRunCommand(commandId: string, context: EditorCommandContext = {}): boolean {
    if (this.historicalPreviewActive && commandId !== 'document.version-history') return false;
  ...
  revealInLayers(nodeId: string): boolean {
    if (!findNode(this.doc, nodeId) || !this.openPanel('layers')) return false;
    this.select(nodeId);
  ...
  addBlock(blockId: string, parentId: string, index?: number, slot?: string): ComponentNode | null {
    ...
    if (block.insert) return block.insert({ editor: this, parentId, index, slot });
  ...
  insertAssetImage(
    ...
    const asset: Asset | undefined = this.engine.doc.assets.find((entry) => entry.id === assetId);
    if (!asset || (asset.type !== 'image' && asset.type !== 'svg')) return null;
  ...
  applyAssetToImage(assetId: string, nodeId: string): boolean {
    ...
    if (!asset || (asset.type !== 'image' && asset.type !== 'svg') || !isImageElementNode(node)) return false;
  ...
  registerInlineExpressionTarget(
    ...
    return () => {
      if (this.#inlineExpressionTarget === target) this.#inlineExpressionTarget = null;
    };
  }
  ...
  insertExpression(expression: string): boolean {
    ...
    if (this.editingId) {
      ...
      if (!target || target.nodeId !== this.editingId || this.selectedId !== this.editingId) return false;
  ...
  updatePaginationRules(
    ...
    for (const [key, value] of Object.entries(pagination)) {
      if (value === undefined || value === false) delete pagination[key as keyof typeof pagination];
    }
    this.engine.batch([
      ...target.commands,
      {
  ...
  setStyleBinding(nodeId: string, property: string, value: string | undefined): void {
    ...
    const node = findNode(this.engine.doc, target.nodeId);
    if (!node) return;
    const bindings = (node.styleBindings ? structuredClone(node.styleBindings) : []).filter(
      (b) => !(b.property === property && b.value !== undefined),
    );
    if (value !== undefined && value !== '') bindings.push({ property, value });
    this.engine.batch([
      ...target.commands,
      {
        type: 'component:update',
  ...
  updatePrintVariantForPage(
    ...
    this.engine.batch(planUpdatePrintVariant(
      this.engine.doc,
      position,
      pageIndex,
      variant,
  ...
  setPrintSectionLinked(
    ...
    this.engine.batch(planSetPrintSectionLinked(
      this.engine.doc,
      sectionId,
      position,
      linkedToPrevious,
...
export function getEditorContext(): EditorContext {
  const editor = getContext<EditorContext | undefined>(EDITOR_CONTEXT_KEY);
  if (!editor) {

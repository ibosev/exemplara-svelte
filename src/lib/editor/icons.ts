...
const BUILTIN_COMPONENT_ICONS: Readonly<Record<string, ComponentIcon>> = {
  text: Type,
  image: Image,
  container: Box,
  columns: Columns2,
...
function iconRegistryFrom(entries: Iterable<[string, ComponentIcon]>): ComponentIconRegistry {
...
export const legacyComponentIconRegistry = iconRegistryFrom(
  Object.entries(BUILTIN_COMPONENT_ICONS),
);

export function createComponentIconRegistry(
  inheritLegacy = false,
): ComponentIconRegistry {
  return iconRegistryFrom(
    inheritLegacy
...
export function componentIcon(type: string): typeof IconType | null {
  return legacyComponentIconRegistry.get(type);
}

/** Register a Lucide icon for a custom component type. */
export function registerComponentIcon(type: string, icon: typeof IconType): void {

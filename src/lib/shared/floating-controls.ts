 ...
 */
export function placeFloatingControls(
  target: RectEdges,
  page: RectEdges,
  viewport: RectEdges,
  fullWidth: number,
  ...
  const y = clamp(
    target.top,
    viewport.top + gap,
    Math.max(viewport.top + gap, viewport.bottom - height - gap),
  );
  const rightX = page.right + gap;
  const rightSpace = viewport.right - gap - rightX;
  if (rightSpace >= fullWidth) return { mode: 'full', side: 'right', x: rightX, y };

  const leftX = page.left - gap - fullWidth;
  const leftSpace = page.left - gap - (viewport.left + gap);
  if (leftSpace >= fullWidth) return { mode: 'full', side: 'left', x: leftX, y };

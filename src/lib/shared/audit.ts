...
export function auditDocument(doc: ExemplaraDocument, options: AuditOptions = {}): AuditIssue[] {
  ...
  for (const page of doc.pages) {
    ...
    function walk(nodes: ComponentNode[], background: string): void {
      for (const node of nodes) {
        if (node.hidden) continue;

        const ownBackground =
          typeof node.props.background === 'string' && node.props.background
        ...
        if (node.children) walk(node.children, ownBackground);
    ...
    function checkText(node: ComponentNode, background: string): void {
    ...
    function checkImage(node: ComponentNode): void {
      if (!node.props.src) {
        issues.push({
          severity: 'error',
          rule: 'image-src',

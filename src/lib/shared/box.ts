...
export const ZERO_BOX: BoxValues = { top: 0, right: 0, bottom: 0, left: 0 };

/** Parse a CSS shorthand ("8px", "4px 8px", …) into per-side values. */
export function parseBoxValues(value: string | BoxValues | undefined | null): BoxValues {
  if (!value) return { ...ZERO_BOX };
...
export function formatBoxValues(box: BoxValues, unit = 'px'): string {
...
export function isZeroBox(box: BoxValues): boolean {

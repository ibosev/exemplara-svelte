...
export function sanitizePrintTemplateHtml(html: string): string {
  const styleBlocks: string[] = [];
  const styleToken = (index: number) => `__EXEMPLARA_PRINT_STYLE_${index}__`;
  let output = html
  ...
  output = output.replace(/<\/?([a-z][a-z0-9]*)\b([^>]*)>/gi, (match, rawTag: string, rawAttrs: string) => {
    ...
    while ((attrMatch = attrPattern.exec(rawAttrs)) !== null) {
      ...
      if (name === 'style') value = safeInlineStyle(value);
  ...
  return output.replace(/__EXEMPLARA_PRINT_STYLE_(\d+)__/g, (_match, rawIndex: string) => {
...
export function sanitizePrintTemplateCss(css: string): string {
  return css
...
export function resolvePrintTemplateVariant(
  template: PrintTemplate,
  pageIndex: number,
): { variant: PrintTemplateVariant; html: string } {
...
export function renderPrintTemplateHtml(
  doc: ExemplaraDocument,
  position: PrintTemplatePosition,
  context: PrintTemplateContext,
): { html: string; variant: PrintTemplateVariant } {
  const template = doc.print[position];
  if (!doc.print.enabled || !template.enabled) return { html: '', variant: 'default' };
  const resolved = resolveSectionTemplate(doc, position, context.pageIndex);
  return {
    html: sanitizePrintTemplateHtml(
      context.resolveData === false
        ? resolved.html
...
function renderPrintTemplateStyles(doc: ExemplaraDocument, css = doc.print.css): string {
  return [
    TEMPLATE_BASE_CSS,
...
function renderTemplateDocumentShell(
  doc: ExemplaraDocument,
  position: PrintTemplatePosition,
  renderedHtml: string,
...
export function renderPrintTemplateDocument(
  doc: ExemplaraDocument,
  position: PrintTemplatePosition,
  context: PrintTemplateContext,
): string {
  const rendered = renderPrintTemplateHtml(doc, position, context);
  return renderTemplateDocumentShell(
    doc,
    position,
    rendered.html,
    rendered.variant,
...
export function renderPrintTemplateSourceDocument(
  doc: ExemplaraDocument,
  position: PrintTemplatePosition,
  html: string,
  css: string,
  context: PrintTemplateContext,
  variant: PrintTemplateVariant = 'default',
): string {
  const renderedHtml = sanitizePrintTemplateHtml(
    context.resolveData === false
      ? html
...
export function renderEditablePrintTemplateDocument(
  doc: ExemplaraDocument,
  position: PrintTemplatePosition,
  html: string,
): string {
...
export function renderBrowserPrintTemplate(
  doc: ExemplaraDocument,
  position: PrintTemplatePosition,
  context: PrintTemplateContext,
  horizontalMargins: Pick<BoxSpacing, 'left' | 'right'>,
): string {
  const rendered = renderPrintTemplateHtml(doc, position, context);
  const css = renderPrintTemplateStyles(doc);
...
export function effectivePrintMargins(
  doc: ExemplaraDocument,
  base: BoxSpacing,
): BoxSpacing {
  if (!doc.print.enabled) return { ...base };
  return {
    ...base,

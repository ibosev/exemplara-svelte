...
function createMinimalPrintSettings() {
...
function createEditorialPrintSettings() {
    const settings = createPrintSettings();
...
export function createPrintPreset(id) {
    const preset = PRINT_PRESETS.find((candidate) => candidate.id === id);
    if (!preset)
        throw new Error(`Unknown print preset: ${String(id)}`);

...
function fragmentsInGroup(doc, groupId) {
    const fragments = [];
    visitNodes(doc, (node) => {
        if (node.flow?.groupId === groupId)
            fragments.push(node);
    });
    return fragments;
}
export function flowFragmentCount(doc) {
    let count = 0;
    visitNodes(doc, (node) => {
        if (node.flow && !findParentNode(doc, node.id)?.flow)
            count += 1;
    });
    return count;
}
function mergedTextContent(fragments) {
    const original = fragments[0]?.flow?.originalContent;
    if (original !== undefined)
        return original;
    return fragments.map((fragment) => String(fragment.props.content ?? '')).join('');
}
function enclosingContainerFlowGroup(doc, node) {
    let current = node;
    let groupId = null;
    while (current) {
        const parent = findParentNode(doc, current.id);
        if (parent?.flow?.kind === 'container')
            groupId = parent.flow.groupId;
        current = parent;
    }
    return groupId;
}
function mergeFlowPair(head, tail) {
...
function mergedFlow(head, tail, remainingFragments) {
    if (!head.flow || !tail.flow || remainingFragments <= 1)
        return undefined;
...
export function planAdjacentFragmentMerge(doc) {
...
function containerFlow(node, continuation) {
    return {
...
function partitionContainerAtTarget(node, split) {
...
function splitContainerFlowNode(node, split) {
...
function splitLeafFlowNode(node, split) {
    if (node.pagination?.keepTogether)
        return null;
    const groupId = node.flow?.groupId ?? node.id;
    const tail = cloneNodeDeep(node);
    if (split.kind === 'text') {
        if (!split.headContent || !split.tailContent)
            return null;
    ...
    if (split.splitIndex <= 0 || split.splitIndex >= split.rowCount)
        return null;
...
export function splitFlowNode(node, split) {

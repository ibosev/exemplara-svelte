import { derivedPanelToken, derivedSurfaceToken, harmonizeTemplateThemeCss, harmonizeThemeColorValue, isDarkColorSample, isPageBackgroundTokenName, retieBakedRootBackgrounds, stripCssCustomProperties, themePrefixFromPageToken, } from '../core/custom-css-tokens.js';
...
function safeCssValue(value, allowImageUrls = false) {
    if (value.length > 1000 || /[<>\\]/.test(value) || /\/\*|\*\//.test(value))
        return false;
    if (/(?:@import|behavior\s*:|expression\s*\(|image-set\s*\(|cross-fade\s*\(|element\s*\(|paint\s*\(|src\s*\()/i.test(value))
        return false;
    let remainder = value;
    ...
    if (urls.length > 0) {
        ...
        for (const match of urls) {
            ...
            if (!/^(?:(?:https:\/\/|data:image\/(?:png|jpeg|gif|webp);base64,|\/|\.\.?\/)[^\s]+|\{\{[A-Za-z_$][A-Za-z0-9_$.-]*\}\})$/i.test(url))
                return false;
        }
...
export function sanitizeImportedDeclarations(source, allowImageUrls = false) {
    const properties = {};
...
function importedRules(source, warnings) {
    ...
    while (cursor < cleaned.length) {
        ...
        for (; close < cleaned.length && depth > 0; close++) {
            ...
            if (quote) {
                if (character === quote && cleaned[close - 1] !== '\\')
                    quote = '';
                continue;
            }
...
function declarationsFor(element, classRules) {
    const declarations = {};
    for (const className of Array.from(element.classList)) {
        Object.assign(declarations, classRules.get(className)?.properties ?? {});
    }
    return Object.assign(declarations, sanitizeImportedDeclarations(element.getAttribute('style') ?? ''));
}
function onlyLayoutElementChild(element) {
    const meaningfulChildren = Array.from(element.childNodes).filter((child) => child.nodeType === 1 || (child.nodeType === 3 && !!child.textContent?.trim()));
...
export function groupImportedPageNodes(imported, hasPageRoot, pageStrategy = 'auto') {
    if (pageStrategy === 'single')
        return [imported.map((entry) => entry.node)];
    const structuralNodes = imported.filter((entry) => PAGE_SECTION_ELEMENTS.has(entry.tag)).length;
    const splitIntoPages = imported.length >= 2 && (hasPageRoot || structuralNodes >= Math.ceil(imported.length * 0.6));
...
function styleBindingsFor(element, classRules) {
    const bindings = [];
    for (const className of Array.from(element.classList)) {
    ...
    for (const [property, value] of Object.entries(sanitizeImportedDeclarations(element.getAttribute('style') ?? ''))) {
        bindings.push({ property, value });
    }
    return bindings;
}
function applyBindings(node, element, classRules, options = {}) {
    ...
    if (!conditionalNames.has('id') && htmlId && !htmlId.includes('{{'))
        node.props.htmlId = htmlId;
    if (conditionalNames.has('id'))
        node.props.previewHtmlId = element.getAttribute('id') ?? '';
...
function importedRawAttributes(element) {
    const attributes = {};
    ...
    if (raw) {
        try {
            ...
            for (const [name, value] of Object.entries(parsed)) {
                if (typeof value === 'string' && !name.toLowerCase().startsWith('on'))
...
function importedRawAttributeBlocks(element) {
...
function attributeNamesInBlocks(blocks) {
    const names = new Set();
    for (const block of blocks) {
...
function inlineStyleText(element, allowRemoteImages, preserveTemplateExpressions = false) {
...
function safeLosslessInlineStyle(source) {
...
function losslessElementNode(element, classRules, warnings, allowRemoteImages) {
    const attributeBlocks = importedRawAttributeBlocks(element);
    const conditionalNames = attributeNamesInBlocks(attributeBlocks);
    const attributes = importedRawAttributes(element);
    const previewAttributes = {};
...
function textNodeFromElement(element, classRules, native = false) {
...
function staticControlNode(element, classRules, warnings) {
    ...
    if (!content)
        content = tag === 'button' ? 'Button' : 'Empty field';
...
function embeddedContentPlaceholder(element, classRules, warnings) {
...
function tableNode(element, classRules, warnings) {
    const tableRows = Array.from(element.querySelectorAll('tr'));
    if (tableRows.length === 0)
        return null;
    const cells = tableRows.map((row) => Array.from(row.querySelectorAll('th, td')).map((cell) => cell.textContent?.trim() ?? ''));
    ...
    const rows = cells.map((row) => Object.fromEntries(columns.map((column, index) => [column.key, row[index] ?? ''])));
...
function numericCssValue(element, property, fallback = 0) {
...
function directAnnotatedChild(element, tagName, attribute, value) {
 ...
 */
function containsOnlySkippedHeadContent(element) {
...
function convertElement(element, classRules, warnings, allowRemoteImages, web, native = false) {
    const tag = element.tagName.toLowerCase();
    if (web && tag === 'shb-if') {
        ...
        const fallbackContainsOnlySkippedHeadContent = fallbackBranch
            ...
            : false;
        if ((branchContainsOnlySkippedHeadContent || fallbackContainsOnlySkippedHeadContent)
        ...
        const fallback = fallbackBranch
            ...
            : [];
        if (!field || (children.length === 0 && fallback.length === 0))
            return children[0] ?? fallback[0] ?? null;
    ...
    if (tag === 'iframe')
        return embeddedContentPlaceholder(element, classRules, warnings);
    if (SKIPPED_ELEMENTS.has(tag))
        return null;
    if (web && !native)
        return losslessElementNode(element, classRules, warnings, allowRemoteImages);
    if (FORM_CONTROL_ELEMENTS.has(tag))
        return staticControlNode(element, classRules, warnings);
    if (RICH_TEXT_ELEMENTS.has(tag)
    ...
    if (tag === 'hr')
        return applyBindings(createNode('divider', {
            thickness: 1, color: '#cccccc', style: 'solid',
        }), element, classRules);
    if (tag === 'table')
        return tableNode(element, classRules, warnings);
    if (tag === 'img') {
    ...
    const rawClass = rawImportedAttribute(element, 'class') ?? '';
    if (web && tag === 'a' && !(native && (element.children.length > 0
    ...
    if (children.length === 0 && Object.keys(inline).length === 0) {
        const hasIdentity = Boolean(element.classList.length
            || element.id
...
function flexDirectionFromInline(inline) {
    const explicit = (inline['flex-direction'] ?? '').toLowerCase();
    if (explicit === 'column' || explicit === 'column-reverse')
        return 'column';
    if (explicit === 'row' || explicit === 'row-reverse')
        return 'row';
...
function convertChildNodes(element, classRules, warnings, allowRemoteImages, web, native = false) {
    const children = [];
    for (const child of Array.from(element.childNodes)) {
        if (child.nodeType === 3 && child.textContent) {
            ...
            const converted = convertElement(child, classRules, warnings, allowRemoteImages, web, native);
            if (converted)
...
export function importHtmlCss(markup, options = {}) {
    if (typeof DOMParser === 'undefined') {
        throw new Error('HTML import is only available in a browser.');
    }
    if (!markup.trim())
        throw new Error('Paste or select an HTML document to import.');
    const parsed = new DOMParser().parseFromString(markup, 'text/html');
    const warnings = [];
    const embeddedCss = Array.from(parsed.querySelectorAll('style'))
        .map((style) => style.textContent ?? '')
        .join('\n');
    const { rules, classRules, rootProperties } = importedRules(`${embeddedCss}\n${options.css ?? ''}`, warnings);
    ...
    const declaredPageRoot = bodyElements.length === 1 && PAGE_ROOT_ELEMENTS.has(bodyElements[0].tagName.toLowerCase())
        ? bodyElements[0]
        : parsed.body;
    const pageStyleElements = declaredPageRoot === parsed.body ? [] : [declaredPageRoot];
    ...
    for (const child of Array.from(pageRoot.childNodes)) {
        if (child.nodeType === 3 && child.textContent) {
            ...
            const converted = convertElement(child, classRules, warnings, options.allowRemoteImages !== false, options.web === true, options.native === true);
            if (converted)
                imported.push({ node: converted, tag: child.tagName.toLowerCase() });
        }
    }
    ...
    const groups = groupImportedPageNodes(imported, pageRoot !== parsed.body, options.pageStrategy);
    groups.forEach((children, index) => {
        ...
        if (Object.keys(pageRootStyles).length > 0)
            page.regions.body.style = { ...pageRootStyles };
...
function classifyImportedCssVariable(cssVar, value) {
    const trimmed = value.trim();
    if (!trimmed)
        return 'skip';
    const name = cssVar.toLowerCase();
    if (name.includes('font-heading') || (name.includes('font') && name.includes('head')))
        return 'heading-font';
    if (name.includes('font-body') || (name.includes('font') && name.includes('body')))
        return 'body-font';
    if (name.includes('font-size') || name.endsWith('-size') && name.includes('font'))
        return 'font-size';
    if (/^#([0-9a-f]{3,8})$/i.test(trimmed) || /^(?:rgba?|hsla?)\(/i.test(trimmed))
        return 'color';
    if (/^var\(--color-[A-Za-z0-9_-]+\)\s*$/i.test(trimmed))
        return 'bound-color';
    return 'skip';
}
function rewriteCssVariableAssignment(css, cssVar, tokenVar) {
    const escaped = cssVar.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    return css.replace(new RegExp(`(${escaped}\\s*:\\s*)([^;}]+)`, 'gi'), (full, prefix, value) => (value.trim().toLowerCase().startsWith('var(') ? full : `${prefix}var(${tokenVar})`));
}
/**
 ...
 */
export function bindImportedCssToDesignTokens(document) {
    if (document.meta.medium !== 'web')
        return false;
    ...
    const nextColors = { ...tokens.colors };
    let nextCss = css;
    let headingFont;
    let bodyFont;
    let bodySize;
    ...
    const prefixes = new Set(Object.keys(tokens.colors)
    ...
    for (const prefix of prefixes) {
        ...
        if (bgValue && isDarkColorSample(bgValue) === false)
            continue;

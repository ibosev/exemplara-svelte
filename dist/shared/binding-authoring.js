import { extractSampleDataPaths, resolveDataPath, } from './data-browser.js';
/** Canonical authoring syntax shared by inline editors and future plugins. */
export function bindingExpression(path) {
    return `{{${path}}}`;
}
/**
 ...
 */
export function findBindingAutocompleteTrigger(text, caretOffset) {
    const end = Math.max(0, Math.min(caretOffset, text.length));
    const before = text.slice(0, end);
    const start = before.lastIndexOf('{{');
    if (start < 0 || before.lastIndexOf('}}') > start)
        return null;
    ...
    if (pipe >= 0) {
        ...
        if (!expressionPrefix || query.includes('|'))
            return null;
...
export function completeBindingAutocomplete(text, trigger, path) {
 ...
 */
export function bindingAutocompleteSuggestions(sources, query = '', limit = 12) {
...
export function bindingAuthoringValue(node, targetProp = 'content') {
...
export function parseExactBindingExpression(value) {
    const plain = value
        .replace(/<br\s*\/?\s*>/gi, '\n')
        .replace(/<[^>]*>/g, '')
        .trim();
...
export function dataSourceIdForPath(sources, path, preferredSourceId) {
    const preferred = preferredSourceId
        ? sources.find((source) => source.id === preferredSourceId)
        : undefined;
    if (preferred && resolveDataPath(preferred.sampleData, path) !== undefined)
        return preferred.id;
 ...
 */
export function commitBindingAuthoring(node, content, sources, preferredSourceId, targetProp = 'content') {
    ...
    if (path) {
        ...
        if (sourceId) {
            ...
            if (existingIndex >= 0)
                bindings.splice(existingIndex, 1, binding);
            else

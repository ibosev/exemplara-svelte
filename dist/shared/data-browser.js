...
export function sampleDataBySource(sources) {
    const result = {};
    for (const source of sources) {
        if (source.sampleData && typeof source.sampleData === 'object' && !Array.isArray(source.sampleData)) {
            result[source.id] = source.sampleData;
        }
    }
    return result;
}
export function resolveDataPath(value, path) {
    if (!path)
        return value;
    let current = value;
...
export function humanizeDataPath(path) {
    const parts = path.split('.').filter((part) => part !== '' && !/^\d+$/.test(part));
    if (parts.length === 0)
        return 'Field';
    const labeled = parts.slice(-2).map(titleCaseSegment);
    if (labeled.length === 2) {
        const [parent, child] = labeled;
        if (!child.includes(' '))
            return `${parent} ${child.toLowerCase()}`;
        return `${parent} ${child}`;
    }
    return labeled[0];
}
export function friendlyDataType(type) {
    if (type === 'string')
        return 'Text';
    if (type === 'number')
        return 'Number';
...
export function formatDataPreview(value, maxLength = 52) {
    let preview;
    if (value === undefined)
        preview = 'Unresolved';
    else if (value === null)
        ...
        preview = `{ ${Object.keys(value).slice(0, 4).join(', ')}${Object.keys(value).length > 4 ? ', …' : ''} }`;
    else
...
export function extractSampleDataPaths(sources, maxDepth = 5) {
    const paths = [];
    const walk = (source, value, path, depth) => {
        if (path) {
            paths.push({
                sourceId: source.id,
        ...
        if (Array.isArray(value)) {
            if (value.length > 0)
                walk(source, value[0], path ? `${path}.0` : '0', depth + 1);
            return;
        }
        if (typeof value === 'object') {
    ...
    for (const source of sources)
        walk(source, source.sampleData ?? {}, '', 0);
    return paths;
}
function dataTreeNode(source, key, path, value, depth, maxDepth) {
    const children = [];
    if (depth < maxDepth && Array.isArray(value) && value.length > 0) {
        ...
        for (const [childKey, childValue] of Object.entries(value)) {
            children.push(dataTreeNode(source, childKey, path ? `${path}.${childKey}` : childKey, childValue, depth + 1, maxDepth));
        }
    }
    return {
...
export function buildSampleDataTrees(sources, maxDepth = 6) {
    return sources.map((source) => dataTreeNode(source, source.name, '', source.sampleData ?? {}, 0, maxDepth));
}
/** Collect explicit AST DataBinding records with resolved sample-data previews. */
export function collectDocumentBindings(doc) {

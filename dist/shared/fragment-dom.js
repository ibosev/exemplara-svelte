...
function pointAt(nodes, absoluteOffset) {
...
function rangeFor(root, nodes, start, end) {
...
function lineCount(range) {
    ...
    for (const rect of Array.from(range.getClientRects())) {
        if (rect.width <= 0 || rect.height <= 0)
            continue;
        if (!tops.some((top) => Math.abs(top - rect.top) <= LINE_TOLERANCE_PX))
            tops.push(rect.top);
    }
    return tops.length;
}
...
function localLineCounts(root, point) {
    let block = point.node.parentElement;
    while (block && block !== root && !TEXT_BLOCK_TAGS.has(block.tagName))
...
function serializeFragment(root, range) {
    const container = root.ownerDocument.createElement('div');
    container.append(range.cloneContents());
    return sanitizeHtml(container.innerHTML);
}
function wordBoundaryAtOrBefore(text, offset) {
    let candidate = Math.min(offset, text.length - 1);
    while (candidate > 0 && !/\s/.test(text[candidate - 1] ?? ''))
        candidate -= 1;
 ...
 */
export function measureTextFragmentDom(root, node, availableBottom) {
    if (node.type !== 'text' || node.pagination?.keepTogether || node.dataBindings?.length)
        return undefined;
    ...
    if (lastFitting <= 0 || lastFitting >= text.length)
        return undefined;
    ...
    while (splitOffset > 0) {
        ...
        if (localLines.before >= minimumBefore
            ...
            if (!headContent.trim() || !tailContent.trim())
                return undefined;
...
export function measureTableFragmentDom(table, node, availableBottom) {
    if (node.type !== 'table' || node.pagination?.keepTogether)
        return undefined;
    const rows = Array.from(table.querySelectorAll(':scope > tbody > tr'));
    if (rows.length < 2)
        return undefined;
    const firstOverflow = rows.findIndex((row) => row.getBoundingClientRect().bottom > availableBottom);
    if (firstOverflow <= 0 || firstOverflow >= rows.length)
        return undefined;
    return { kind: 'table', splitIndex: firstOverflow, rowCount: rows.length };
}
function renderedContentElement(outer, nodeId, className) {
    return Array.from(outer.children).find((child) => child instanceof HTMLElement
        && child.dataset.nodeId === nodeId
        && child.classList.contains(className));
}
function verticalContainerContent(outer, node) {
    if (node.type !== 'container' || node.pagination?.keepTogether)
        return undefined;
...
function outerChildElement(content, childId) {
    return Array.from(content.children).find((child) => child instanceof HTMLElement
 ...
 */
export function measureContainerFragmentDom(outer, node, availableBottom) {
    ...
    for (const child of node.children ?? []) {
        ...
        if (bottom <= childAvailableBottom + LINE_TOLERANCE_PX)
            continue;
        if (top < childAvailableBottom - LINE_TOLERANCE_PX) {
            ...
            if (!child.pagination?.keepTogether && child.type === 'text') {
                ...
                const targetSplit = text
                    ...
                    : undefined;
                if (targetSplit)
                    return { kind: 'container', targetNodeId: child.id, targetSplit };
            }

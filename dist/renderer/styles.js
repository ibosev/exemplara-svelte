 ...
 */
export function renderDocumentBaseStyles(rootSelector = '.ex-document', pageSelector = `${rootSelector} .ex-page`) {
    return `
${rootSelector} {
...
export function renderDesignTokens(tokens, selector = ':root') {
...
export function renderFontFaces(tokens) {
    return tokens.fonts
        .map((font) => {
        const lines = [
            '@font-face {',
...
function scopedRuleSelector(selector, scope) {
    ...
    if (normalized.startsWith('body '))
        return `${scope} ${normalized.slice('body '.length)}`;
    if (normalized.startsWith('html body '))
        return `${scope} ${normalized.slice('html body '.length)}`;
    if (normalized.startsWith('.ex-document '))
        return `${scope} ${normalized.slice('.ex-document '.length)}`;
    if (normalized === '.ex-page')
        return scope;
    if (normalized.startsWith('.ex-page'))
        return `${scope}${normalized.slice('.ex-page'.length)}`;
    return `${scope} ${normalized}`;
}
export function renderStyleRules(rules, scope) {
    return renderStyleRulesForTarget(rules, scope);
}
function containerQuery(mediaQuery) {
...
function scopeLosslessImportedCss(source, pageSelector) {
    const responsive = source.replace(/@media\s+([^{}]+)\{/gi, (match, query) => {
    ...
    const rootAdjusted = viewportRelative.replace(/(^|[{}])([^{}]+)\{/g, (full, boundary, selectorSource) => {
        if (selectorSource.trim().startsWith('@'))
            return full;
...
function renderStyleRulesForTarget(rules, scope, target = {}) {
    return rules
        .map((rule) => {
...
export function renderScopedDocumentStyles(document, pageSelector) {
    ...
    return [
        ...
        renderStyleRulesForTarget(document.styles.rules, pageSelector, document.meta.medium === 'web' ? { containerName: 'exemplara-page-preview' } : {}),
        customWebCss
            ? (losslessHtml

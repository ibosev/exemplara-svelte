...
import { registerHandlebarsControlRenderers, registerLosslessControlRenderers, registerWebRenderers, WEB_COMPONENT_CSS, } from './web-components.js';
function regionMarkup(region, name, ctx) {
...
export function renderWeb(document, options = {}) {
    if (options.policy)
        assertDocumentCapabilities(document, options.policy);
    const pageIndex = Math.max(0, Math.min(options.pageIndex ?? 0, Math.max(0, document.pages.length - 1)));
    const page = document.pages[pageIndex];
    if (!page)
        throw new Error('Web documents require at least one page');
    ...
    if (!runtime.renderers.get('web-section'))
        registerWebRenderers(runtime.renderers);
    if (options.handlebarsTemplate)
        registerHandlebarsControlRenderers(runtime.renderers);
    else if (losslessHtml)
        registerLosslessControlRenderers(runtime.renderers);
    ...
    const fullHtml = [
        ...
        '</style>',
        '</head>',
        `<body${losslessHtml && pageClassName ? ` class="${escapeAttr(pageClassName)}"` : ''}${losslessHtml && pageHtmlId ? ` id="${escapeAttr(pageHtmlId)}"` : ''}${losslessHtml && pageInlineStyle ? ` style="${escapeAttr(pageInlineStyle)}"` : ''}>`,
        html,
        '</body>',
        '</html>',
    ].filter(Boolean).join('\n');
...
export function renderWebTemplate(document, options = {}) {

export const stringValue = (value, fallback = '') => value === undefined || value === null ? fallback : String(value);
export const numberValue = (value, fallback) => {
    const numeric = Number(value);
    return Number.isFinite(numeric) ? numeric : fallback;
};
/** Merge rule/value bindings over a renderer's built-in presentation. */
export function nodePresentation(node, baseClass, style = {}) {
    ...
    for (const binding of node.styleBindings ?? []) {
        if (binding.ruleId)
            classes.push(`ex-r-${binding.ruleId}`);
        if (binding.value !== undefined)
            merged[binding.property] = binding.value;
    }
    return { classes, style: merged };
}
export function containerPresentation(node) {
    const { direction, gap, padding, background, align, justify, width, maxWidth, minHeight, wrap, color, border, borderRadius, overflow, position, } = node.props;
...
export function columnsPresentation(node) {
...
export function columnPresentation(ratiosValue, gapValue, index) {
    const ratios = Array.isArray(ratiosValue) ? ratiosValue : [1, 1];

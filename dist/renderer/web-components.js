...
export function losslessHtmlAttributes(node, useConditionalPreview = false) {
    const result = {};
    const attributes = node.props.attributes;
    if (attributes && typeof attributes === 'object' && !Array.isArray(attributes)) {
        for (const [name, rawValue] of Object.entries(attributes)) {
            if (!SAFE_IMPORTED_ATTRIBUTE.test(name)
            ...
            if (IMPORTED_URL_ATTRIBUTES.has(name.toLowerCase()) && !safeImportedUrl(value))
                continue;
            result[name] = value;
        }
    }
    ...
    const className = stringValue(useConditionalPreview && node.props.previewClassName ? node.props.previewClassName : node.props.className).trim();
    const htmlId = stringValue(useConditionalPreview && node.props.previewHtmlId ? node.props.previewHtmlId : node.props.htmlId).trim();
    const inlineStyle = stringValue(useConditionalPreview && node.props.previewInlineStyle ? node.props.previewInlineStyle : node.props.inlineStyle).trim();
    const bindingClasses = (node.styleBindings ?? [])
        .map((binding) => binding.ruleId ? `ex-r-${binding.ruleId}` : '')
        .filter(Boolean);
    const bindingStyle = styleString(Object.fromEntries((node.styleBindings ?? [])
...
function exportedAttributeBlocks(node, resolveData) {
    if (resolveData)
        return '';
    if (!Array.isArray(node.props.attributeBlocks))
        return '';
    return node.props.attributeBlocks.filter((value) => typeof value === 'string'
        && !/[<>]/.test(value)
        && !/\bon[a-z0-9_-]*\s*=|(?:javascript|vbscript)\s*:/i.test(value)).join('');
}
function attrs(node, baseClass, style = {}) {
    const presentation = nodePresentation(node, baseClass, style);
    const inline = styleString(presentation.style);
    return `class="${escapeAttr(presentation.classes.join(' '))}" data-node-id="${escapeAttr(node.id)}"${inline ? ` style="${escapeAttr(inline)}"` : ''}`;
}
function safeHref(value) {
    const href = stringValue(value, '#').trim();
    return /^(?:https?:\/\/|mailto:|tel:|\/|#|\{\{)/i.test(href) ? href : '#';
}
export const webSectionDefinition = {
    type: 'web-section',
    label: 'Web section',
    icon: '▦',
    category: 'web',
...
export const webLinkDefinition = {
    type: 'web-link',
    label: 'Button / link',
    icon: '↗',
    category: 'web',
...
export const losslessHtmlElementDefinition = {
    type: 'html-element',
    label: 'HTML element',
    icon: '◇',
    category: 'web',
...
export const losslessHtmlTextDefinition = {
    type: 'html-text',
    label: 'Imported text',
    icon: 'T',
    category: 'internal',
...
export function registerWebRenderers(registry) {
    ...
    registry.register('web-link', (node, _children, ctx) => {
        ...
        const attributeBlocks = exportedAttributeBlocks(node, ctx.resolveData);
        return `<a ${nodeAttrs} href="${escapeAttr(href)}"${ariaLabel ? ` aria-label="${escapeAttr(ariaLabel)}"` : ''}${newTab ? ' target="_blank" rel="noopener noreferrer"' : ''}${attributeBlocks}>${escapeHtml(label)}</a>`;
    });
    registry.register('html-element', (node, children, ctx) => {
        ...
        const attributeBlocks = !ctx.resolveData && Array.isArray(node.props.attributeBlocks)
            ? node.props.attributeBlocks.filter((value) => typeof value === 'string'
                && !/[<>]/.test(value)
                && !/\bon[a-z0-9_-]*\s*=|(?:javascript|vbscript)\s*:/i.test(value)).join('')
            : '';
...
export function registerHandlebarsControlRenderers(registry) {
    registry.register('repeater', (node, _children, ctx) => {
    ...
    registry.register('conditional', (node, _children, ctx) => {
...
export function registerLosslessControlRenderers(registry) {
    registry.register('repeater', (node, _children, ctx) => {
        if (!ctx.resolveData)
            return ctx.renderChildren(node.children ?? [], ctx);
        const path = stringValue(node.props.path);
    ...
    registry.register('conditional', (node, _children, ctx) => {
        ...
        const evaluate = (condition) => evaluateCondition(ctx.dataContext, condition.field, (condition.operator ?? 'exists'), condition.value);
